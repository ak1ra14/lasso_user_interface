...
class QwertyKeyboard(FloatLayout):
    ...
    def __init__(self, title, enter_callback=None, **kwargs):
        ...
        self.flick_mapping =  [
            ...
            ('は', 'ひ', 'ふ', 'へ', 'ほ'),
            ('ま', 'み', 'む', 'め', 'も'),
            ('や', 'ゃ', 'ゆ', 'ぇ', 'よ'),  # adapt for fewer items
            ('ら', 'り', 'る', 'れ', 'ろ'),
            ('わ', 'ゐ', 'ゑ', 'を', '')     # historical kana used as example
        ]
    ...
    def build_flick_keyboard(self):
        ...
        grid = GridLayout(cols=3, spacing=6, size_hint_y=None, height=300)
        for mapping in self.flick_mappings:
            k = FlickKey(mappings=mapping, text_input=self.text_input,
...
class KeyboardApp(App):
    def build(self):
        self.title = "Keyboard App"
        self.screen_manager = ScreenManager()
        self.keyboard_screen = KeyboardScreen(title="Keyboard")
        self.screen_manager.add_widget(self.keyboard_screen)
