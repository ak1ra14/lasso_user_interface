...
from as_utils.as_icons import ToggleButton, CustomSwitch
from as_utils.as_config_loader import load_config, update_current_page, update_text_language, get_valid_value, save_config_partial, update_all_values, check_all_values_same
...
class AlertTypeScreen(SafeScreen):
    def __init__(self, **kwargs):
        ...
        self.ack_config = load_config('as_config/settings.json','ack_json')
        self.ack_enable = get_valid_value(self.ack_config, 'ack_enable', load_config("as_config/settings.json","default_json").get("ack_enable", "yes"))
        self.bed_json = load_config("as_config/settings.json",'bed_json')
        self.fall_json = load_config("as_config/settings.json",'fall_json')
        ...
        self.header = HeaderBar(title="alerts", icon_path="as_images/home.png", button_text="home", button_screen="menu")
        self.buttons = {}
        ...
        image = Image(
            source="as_images/bed_single.png",
            size_hint=(None, None),
            size=(70,70),
            pos = (70, 330)
        )
        ...
        fall_image = Image(
            source="as_images/fall_single.png",
            size_hint=(None, None),
            size=(50,50),
            pos = (420, 335)
        )
...
class SaveButtonAT(IconTextButton):
    def __init__(self, AT_screen, **kwargs):
        super().__init__(**kwargs)
        self.icon_path = "as_images/save.png"
    ...
    def on_press(self):
        ack_config = load_config('as_config/settings.json','ack_json')
        self.buttons = self.AT_screen.buttons
        if ack_config:
            ack_enable = "yes" if self.AT_screen.buttons['ack_button'].switch.active else "no"
            if not check_all_values_same("as_config/settings.json",'ack_json',key='ack_enable', value_to_check=ack_config.get('ack_enable',None)):
                save_config_partial("as_config/settings.json", "ack_json", key='ack_enable',value=ack_enable)
            else:
                update_all_values("as_config/settings.json",'ack_json', key='ack_enable', new_value= ack_enable)   

        config_bed = load_config("as_config/settings.json",'bed_json')
        ...
        save_config_partial('as_config/settings.json','bed_json',key='attach_video',value=attach_video_bed)
        ...
        save_config_partial('as_config/settings.json', 'bed_json', 
                       key='alert_checking', value=alert_checking_bed)

        config_fall = load_config("as_config/settings.json",'fall_json')
        ...
        save_config_partial('as_config/settings.json','fall_json',key='attach_video',value=attach_video_fall)
        ...
        save_config_partial('as_config/settings.json', 'fall_json', 
