...
from as_utils.as_icons import IconTextButton, PageIndicator, CircularImageButton, PageIndicatorWidget
...
class HeaderBar(BoxLayout):
    ...
    '''
    def __init__(self, title="Language", icon_path="as_images/home.png", button_text="home", button_screen="menu", padding=[50, 0, 50, 0], spacing=10, second_button=None, **kwargs,):
        ...
        if second_button:
            ...
            self.add_widget(Widget(size_hint_x=None, width=3))  # Spacer
...
class FooterBar(BoxLayout):
    ...
    def __init__(self, screen_name, **kwargs):
        ...
        left_btn = CircularImageButton(
            image_path="as_images/left_arrow.png",
            diameter=80,
        ...
        self.details = Label(
            text=f"{update_text_language('version')}: {load_config('as_config/V3.json').get('version', 'N/A')} | {update_text_language('device_id')}: {load_config('as_config/V3.json').get('sensor_ID', 'N/A')}",
            font_name='as_fonts/MPLUS1p-Regular.ttf',
            font_size=16,
            size_hint_y=0.05,
            halign='center',
        ...
        right_btn = CircularImageButton(
            image_path="as_images/right_arrow.png",
            diameter=80,
            screen_name=screen_name,
            halign='center'
        )
    ...
    def update_language(self):
        ...
        self.details.text = f"{update_text_language('version')}: {load_config('as_config/V3.json').get('version', 'N/A')} | {update_text_language('device_id')}: {load_config('as_config/V3.json').get('sensor_ID', 'N/A')}"
